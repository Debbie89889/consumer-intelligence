...
Consumer Intelligence 分析專案:用一份真實電商交易資料,做端到端的消費者智慧分析——**客群分群、CLV、Next Best Offer、購買傾向預測**——並已產品化為 **PostgreSQL + FastAPI + Docker + Streamlit** 的可互動服務,再加一層 **grounded LLM 洞察 Copilot**。重點是把「資料 + ML + LLM 連結到商業價值並產品化」做完整且可驗證。
...
專案有兩種截然不同的「狀態」,故意用兩套機制持久化:

- **LangGraph checkpointer**(`copilot_graph/`,見 `PROMPT_langgraph_copilot.md`):負責 **graph 執行的技術性狀態**——目前跑到哪個節點、`interrupt()` 後如何從中斷點 `resume`。這是框架內部的執行序列快照,不是給人看的業務資料。
...
**Human-in-the-Loop 的正當性在於業務,不在技術**

`copilot_graph/campaign_graph.py`(win-back campaign 產生器)在草擬完折扣與文案後,一定會 `interrupt()` 停下來等人工審核,核准後才寫入 DB、產出名單。這不是為了展示 LangGraph 技術炫技——**發折扣給真實客戶(此資料集為 At Risk／Can't Lose Them 543 位高價值沉睡客戶)是有金錢成本、有品牌風險、且一旦寄出就不可逆的行為,必須有人把關**。任何未來要加的 HITL 都要能回答「不可逆的業務動作是什麼」,答不出來就不該加中斷點。

**為何保留既有 LCEL Copilot,不直接改寫成 LangGraph**

`copilot/`(LCEL)與 `copilot_graph/`(LangGraph)兩套敘述層並存,`copilot/` 完全沒被修改過(除了新增 `generate_insight_with_backend`／`get_chat_model` 這種不改變既有行為的 sibling function／改名)。原因:

- `copilot/` 是可運作、有測試、`/customers/{id}/insight` 正式在用的敘述層,砍掉重練只會製造不必要的回歸風險。
- 這次新增 LangGraph 的目的是**展示 LangChain 生態系裡另一種 orchestration 方式**(平行工具調用、條件路由、HITL 中斷/恢復、多輪對話),不是要證明 LCEL「不好」——兩者的定位不同,LCEL 適合單一意圖、無需中斷的場景,LangGraph 適合需要那些額外能力的場景(細節見 README「一種 Copilot 實作的心得」)。
- 兩套實作各自的 grounding 邏輯完全獨立,不會互相污染;之後若要調整其中一套的敘述方式,不會波及另一套的測試或行為。
